function VPL = EvaluateVPL_report(SimulationFolder, NumberOfWells, NumberOfProdLumpsPerWell, NumberOfInjLumpsPerWell, EconomicInputs)

launch_report('ANEPI', SimulationFolder, 'C:\Program Files (x86)\CMG\IMEX\2017.10\Win_x64\EXE\mx201710.exe')

ShutdownTime = 1000000000;

TotalNumberOfWells = sum(NumberOfWells,2);
NumberOfTotalLumps = sum(NumberOfProdLumpsPerWell) + sum(NumberOfInjLumpsPerWell);
NumberOfDates = GetTimeStamp(SimulationFolder)-1;
OutFile = dir(strcat(SimulationFolder, '\\*.out'));
CompleteOutFileName = strcat(OutFile.folder, '\', OutFile.name);

write_data(NumberOfTotalLumps, 'ANEPI', SimulationFolder, SimulationFolder, TotalNumberOfWells)

templateName = strcat(SimulationFolder, '\Time_template.inc');
FileToRead = fopen(templateName);
...
data = read_text(fullfile(SimulationFolder, 'ANEPI_data.txt'), 7, false, true);
